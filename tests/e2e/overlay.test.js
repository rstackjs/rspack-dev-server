...
const { puppeteerArgs } = require('../helpers/puppeteer-constants');
const { runPage } = require('../helpers/run-browser');
...
// Chromium serializes `border: none` with different `border-image` values across versions and platforms.
const normalizeBorderImage = (value) =>
  value.replace(/border-image: (?:initial|none);/g, 'border-image: initial;');
...
const runBrowser = async () => {
  ...
  const page = await runPage(context);
